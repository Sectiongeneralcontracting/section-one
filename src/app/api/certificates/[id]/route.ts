...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  ...
  const before = await prisma.paymentCertificate.findUnique({ where: { id: params.id } });
  if (!before) return NextResponse.json({ error: "Not found" }, { status: 404 });
  if (before.status !== "DRAFT")
    return NextResponse.json({ error: "مينفعش تحذف مستخلص اتقدّم أو اتعمد بالفعل" }, { status: 400 });

  await prisma.paymentCertificate.delete({ where: { id: params.id } });
  await logAudit({ userId: (session.user as any).id, action: "CERTIFICATE_DELETED", entityType: "PaymentCertificate", entityId: params.id, before });
