...
const dict = {
  ar: {
    title: "سلف الموظفين", newAdvance: "سلفة جديدة", cancel: "إلغاء",
    employee: "الموظف *", chooseEmployee: "اختر الموظف", amount: "القيمة *", date: "التاريخ",
    reason: "السبب", save: "حفظ السلفة", saving: "جارٍ الحفظ...", err: "تعذر تسجيل السلفة",
  ...
  en: {
    ...
    reason: "Reason", save: "Save Advance", saving: "Saving...", err: "Failed to record advance",
...
export default function AdvancesPage() {
  ...
  async function load() {
    setLoading(true);
    const [aRes, eRes] = await Promise.all([fetch("/api/employee-advances"), fetch("/api/employees")]);
    ...
    <AppShell
      ...
      action={
        <button onClick={() => setShowForm((v) => !v)} className="bg-primary text-white text-sm px-4 py-2 rounded-xl flex items-center gap-1.5">
          ...
          {showForm ? t.cancel : t.newAdvance}
        </button>
      }
    >
      {showForm && (
        <form onSubmit={handleSubmit} className="card grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <input value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value })} className="w-full border rounded-xl px-3 py-2" />
          </div>
